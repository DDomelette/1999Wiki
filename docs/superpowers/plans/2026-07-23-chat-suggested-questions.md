...
- [x] `SUGGEST-P0-01`：题库为 10–12 条并覆盖约定主题。
- [x] `SUGGEST-P0-02`：最多 4 条、无重复、不修改原数组，小题库安全。
- [x] `SUGGEST-P0-03`：同一挂载期不因草稿、发送、分类或清空而重抽样。
- [x] `INPUT-P0-01`：选择后覆盖草稿并聚焦输入框。
- [x] `INPUT-P0-02`：选择阶段不调用 send、不新增消息。
- [x] `INPUT-P0-03`：空/非空对话均常驻，发送中禁用且结束后恢复。
- [x] `INPUT-P0-04`：发送、重试、检索模式、分类和清空无回归。
- [x] `UI-P0-01`：位置正确且不遮挡现有控件。
- [x] `UI-P0-02`：窄屏横向滚动且不持续挤压消息区。
- [x] `UI-P0-03`：明暗主题均清晰，未引入单主题硬编码色块。
- [x] `A11Y-P0-01`：分组命名、按钮语义、键盘与焦点状态可用。
- [x] 相关 Vitest 已先观察 RED，再完成 GREEN。
- [x] 完整 `npm test` 通过：52 个测试文件、250 项测试。
- [x] `npm run build` 通过；仅保留既有的 500 kB chunk size warning。
- [ ] 真实 RAG 回答链路：环境阻塞，`/health` 返回 `llm_ready=false`、`vectorstore_loaded=false`、`provenance_status=blocked`。
- [x] Mock SSE、2048×1157/390×844、明暗主题、Tab/Enter/Space 与可见焦点验收已记录在 Playwright 用例和截图中。
- [x] P1 均未执行；P2 未进入实现。
